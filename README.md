Serverless DevOps-based shell app for RDS Aurora PostgreSQL database deployment
==================

**CI/CD status**:
![](https://github.com/DamZiobro/serverless-aws-rest-api/workflows/CI/badge.svg)

This is the skeleton app for deploying RDS Aurora PostgreSQL datbase using
...
Additionally, this simple project is demonstration of multiple modern technologies/methodologies/principles:
  ...
  * **Serverless** (Serverless Framework) - AWS Lambda, RDS Aurora
  ...
  * **RESTful API** - simple implementation of REST API based on Python in api directory
...
3. Export ENV=dev to deploy to `dev` stage/environment (by default ENV value is
   get based on feature branch name created according to `git flow` rules 
   or it is HEAD if branch name is too short):
```
export ENV=dev
```
...
make logs
```
5. **Destroy app and RDS database** - delete all AWS resources of your app
```
make destroy-all
```
